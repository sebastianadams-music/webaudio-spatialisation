...
// create receivers and panners
  for (let i = 0; i < numberOfRadios; i++){
    ...
    receivers.push(newReceiver(sources, panners[i])) 
...
function newReceiver(sources, panNode){
  // receives sources
  ...
  for (let i = 0; i < sources.length; i++){
    ...
    sources[i%sources.length].connect(node)
    g.push(node)
...
function radioPosition(circle, index){
  for (let i = 0; i < receivers.length; i++){
    ...
    for (let j = 0; j < receivers[i].length; j++){
